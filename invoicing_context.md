...
&nbsp;&nbsp;swissQrCode | `hash` | swissQrCode; **optional** (however, when 'swissQrCode' is included, then the following top-level fields MUST be empty or omitted: 'iban', 'esrReference' and 'esrParticipantNumber', 'totalGrossAmount' - if 'notes' is entered and 'addInf.ustrd' is empty then 'notes' will be forwarded in the payment field 'addInf.ustrd'/mitteilung) See the [SwissQRCode Section](#minimal-swiss-qr-code-when-creating-an-invoice) at the end of this page to understand Garaio REM Invoicing with SwissQrCodes.
