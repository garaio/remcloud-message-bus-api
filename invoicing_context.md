...
&nbsp;&nbsp;iban | string | iban for the payment (optional); always pass the iban or swissQrCodeText, if you pass the iban field esr attributes and swissQrCodeText must be empty
...
&nbsp;&nbsp;swissQrCodeText | string | The text output from a QR Code scan (optional, when given then ESR and IBAN must be empty)
